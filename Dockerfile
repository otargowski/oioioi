...
RUN apt-get update && \
    apt-get install --no-install-recommends -y \
        ...
        nodejs \
        npm && \
    apt-get clean && apt-get distclean && \
...
# When BuildKit will be more widespread as the default in docker installations,
# this additional stage can be removed by using `RUN --mount`.
